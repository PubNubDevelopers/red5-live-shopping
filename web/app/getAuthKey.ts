export async function getAuthKey(
    ...
    try {
      const TOKEN_SERVER =
        `https://devrel-demos-access-manager.netlify.app/.netlify/functions/api/pillar-ecommerce${isGuidedDemo ? `-guided${customSuffix}` : ''}`;
      //const TOKEN_SERVER =
      //  `http://localhost:8083/.netlify/functions/api/pillar-ecommerce${isGuidedDemo ? '-guided' : ''}${customSuffix}`;
